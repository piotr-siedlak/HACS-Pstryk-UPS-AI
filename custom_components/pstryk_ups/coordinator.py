...
class PstrykUPSCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._pstryk = PstrykAPIClient(api_key=self.config[CONF_PSTRYK_API_KEY], session=session)
        ups_cfg = {
        ...
        self._planner = ClaudePlanner(api_key=self.config[CONF_CLAUDE_API_KEY], ups_config=ups_cfg)
    ...
    async def async_setup_mqtt(self) -> None:
        ...
        if power_topic:
            self._mqtt_unsubs.append(await mqtt.async_subscribe(self.hass, power_topic, self._handle_power_message))
        if history_topic:
            self._mqtt_unsubs.append(await mqtt.async_subscribe(self.hass, history_topic, self._handle_history_message))
        if battery_topic:
            self._mqtt_unsubs.append(await mqtt.async_subscribe(self.hass, battery_topic, self._handle_battery_message))
    ...
    def _handle_power_message(self, msg: mqtt.ReceiveMessage) -> None:
        ...
        try:
            ...
            if payload.startswith("{"):
                ...
                value = data.get("power") or data.get("power_kw") or data.get("value") or data.get("kw") or 0.0
    ...
    def _handle_battery_message(self, msg: mqtt.ReceiveMessage) -> None:
        ...
        try:
            ...
            if payload.startswith("{"):
                data = json.loads(payload)
                value = data.get("battery_level") or data.get("soc") or data.get("state_of_charge") or data.get("percent") or data.get("value") or 50.0
    ...
    def _needs_price_refresh(self, now_utc: datetime, now_warsaw: datetime) -> bool:
        ...
        if now_warsaw.hour >= NEXT_DAY_PRICES_HOUR and self._next_day_prices_fetched_date != today_warsaw:
            return True
        return False
    ...
    def _update_savings_estimate(self) -> None:
        ...
        savings = 0.0
        for item in self.schedule:
            if not item.get("hour", "").startswith(today_prefix):
                continue
            action = item.get("action", ACTION_IDLE)
            price = item.get("price_pln_kwh", 0.0)
            energy_kwh = abs(item.get("power_kw", 0.0)) * 1.0
    ...
    def _build_state_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "next_charge_window": self._get_next_window(ACTION_CHARGE),
            "next_discharge_window": self._get_next_window(ACTION_DISCHARGE),
            ...
            "pstryk_api_last_success": self.pstryk_api_last_success.isoformat() if self.pstryk_api_last_success else None,
            "pstryk_api_last_checked": self.pstryk_api_last_checked.isoformat() if self.pstryk_api_last_checked else None,
            "claude_api_status": self.claude_api_status,
            "claude_api_last_error": self.claude_api_last_error,
            "claude_api_last_success": self.claude_api_last_success.isoformat() if self.claude_api_last_success else None,
    ...
    def _get_current_price(self) -> float | None:
        ...
        for p in self.prices:
            if p.get("timestamp", "")[:13] == current_hour_key[:13]:
                return p.get("price")
        return None
